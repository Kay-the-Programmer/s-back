...
export class StoreInitService {
    async initializeNewStore(storeId: string, storeName: string) {
        console.log(`Initializing store: ${storeName} (${storeId})`);

        await this.seedSettings(storeId, storeName);
        const accountsMap = await this.seedAccounts(storeId);
        await this.seedCategories(storeId);
    ...
    private async seedCategories(storeId: string) {
        const categoriesData = categoryHierarchy;
