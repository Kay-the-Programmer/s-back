...
export const createPurchaseOrder = async (req: express.Request, res: express.Response) => {
    ...
    try {
        ...
        // Automatic Invoice creation if status is ordered
        if (newPO.status === 'ordered') {
            await accountingService.ensureSupplierInvoiceForPO(newPO, client, storeId);
        }
...
export const updatePurchaseOrder = async (req: express.Request, res: express.Response) => {
    ...
    try {
        ...
        const oldPOResult = await client.query('SELECT status FROM purchase_orders WHERE id = $1 AND store_id = $2', [id, storeId]);
        ...
        await client.query('DELETE FROM purchase_order_items WHERE po_id = $1 AND store_id = $2', [id, storeId]);
        for (const item of items) {
            await client.query(
                'INSERT INTO purchase_order_items (po_id, product_id, product_name, sku, quantity, cost_price, received_quantity, store_id) VALUES ($1, $2, $3, $4, $5, $6, $7, $8)',
        ...
        // Automatic Invoice creation if status changed to ordered or is ordered
        if (updatedPO.status === 'ordered') {
            await accountingService.ensureSupplierInvoiceForPO(updatedPO, client, storeId);
        }
...
export const receiveItems = async (req: express.Request, res: express.Response) => {
    ...
    try {
        ...
        const poCheck = await client.query('SELECT id FROM purchase_orders WHERE id = $1 AND store_id = $2', [id, storeId]);
        ...
        // Update PO status
        const itemsResult = await client.query('SELECT quantity, received_quantity, cost_price, product_id FROM purchase_order_items WHERE po_id = $1 AND store_id = $2', [id, storeId]);
        const allReceived = itemsResult.rows.every((item: any) => item.received_quantity >= item.quantity);
        const newStatus = allReceived ? 'received' : 'partially_received';
        const receivedAt = new Date().toISOString();

        const updatedPOResult = await client.query('UPDATE purchase_orders SET status = $1, received_at = $2 WHERE id = $3 AND store_id = $4 RETURNING *', [newStatus, receivedAt, id, storeId]);
        const updatedPO = toCamelCase(updatedPOResult.rows[0]);

        // Record the physical reception in the journal (Debit Inventory, Credit AP)
        // We only record the journal entry for the items JUST received in this call
        await accountingService.recordPurchaseOrderReception(
            id,
            updatedPO.poNumber,
            receivedItems.map(ri => {
                const itemData = itemsResult.rows.find((row: any) => row.product_id === ri.productId);
                return {
                    productId: ri.productId,
                    quantity: ri.quantity,
                    costPrice: itemData ? parseFloat(itemData.cost_price) : 0
                };
            }),
            client,
            storeId
